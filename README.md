...
- Orchestrator 蜂群调度：开发任务进入 Task Board 并派给 worker；Brain 只做规划和状态回传，不进入默认 worker 池。
...
Brain session 只作为规划和状态回传对象，默认不会进入 worker 派发池。开发任务必须由非 Brain worker 执行；如果 headless adapter 不可用，CLI Deck 会提示创建 worker，而不是把任务塞回 Brain。

默认内置一个 headless `Codex Exec` worker。开发任务会优先通过 `codex.cmd exec` 非交互执行，CLI Deck 收集 stdout/stderr/exit code 后更新 Task Board；PTY worker 只是兼容 fallback。
...
Task Board 会持久化到 Electron `userData` 下的 `orchestrator-board.json`。应用重启后会恢复 tasks/runs/events；上次处于 `running` 的任务会被 reclaim 为 `blocked`，避免静默丢失。

Dispatcher 会在 Auto 开启时持续扫描任务板。`ready` task 会按 capability 自动派给 worker；因为“没有 worker”或“应用重启”进入 `blocked` 的 task，会在可用 worker 出现后自动 retry/dispatch。需要人工决策的 `blocked` task 不会被自动重试。

worker 失败产生的 `blocked` 不会自动派生 research 任务；失败会停留在原 task 上，保留输出和 Retry 入口，避免任务越调越乱。
